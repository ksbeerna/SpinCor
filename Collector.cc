...
std::vector<std::vector<float> > eire::Collector::CollectLikelihoods(std::string name)
{
  ...
  if(name == "WJets_cuts"){number_of_files = 2000;
    Cor = "/afs/cern.ch/work/k/ksbeerna/public/weights-cuts/Wbjets_cor/weights-";
    UnCor = "/afs/cern.ch/work/k/ksbeerna/public/weights-cuts/Wbjets_uncor/weights-";
  }
  ...
  if(name == "WJets_smear"){number_of_files = 937;
    ...
    if(use_HitFit){
      infile = new TFile("/data/input/HitFit/ComboOnly/HitFit_WJets.root","OPEN");
      HitFitTree = (TTree*) infile->GetDirectory("HitFit")->Get("HitFitResults");
    }
  }
    ...
    if(use_HitFit){
      infile = new TFile("/data/input/HitFit/ComboOnly/HitFit_Cor.root","OPEN");
      HitFitTree = (TTree*) infile->GetDirectory("HitFit")->Get("HitFitResults");
    }
  }
  ...
  for(int i= 0; i < number_of_files; i++){
	...
	while ( myInfile>>event_number){
	  ...
	  else if(atof(likelihood.c_str()) != 0 && !use_HitFit){L = log(atof(likelihood.c_str()));}
	  else{L = 999;}
  ...
  for(unsigned int j = 0; j < cor.size(); j++){
    if((cor)[j] == 999 || (uncor)[j] == 999 || (-1)*(cor)[j] > Like_cut_up || (-1)*(cor)[j] < Like_cut_down || (-1)*(uncor)[j] > Like_cutuncor_up || (-1)*(uncor)[j] < Like_cutuncor_down){cor.erase(cor.begin()+j);uncor.erase(uncor.begin()+j);j--;}
  }
